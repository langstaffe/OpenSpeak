...
func TestWebSettingsRouteAndSessionInvalidation(t *testing.T) {
	...
	externalBootstrap := httptest.NewRecorder()
	env.server.ServeHTTP(externalBootstrap, httptest.NewRequest(http.MethodGet, "https://example.test/chat/flutter_bootstrap.js", nil))
	wantExternalBase := `const base = "https://static.example.test/root/assets-v-` + assetVersion + `/";`
	if externalBootstrap.Code != http.StatusOK || !strings.Contains(externalBootstrap.Body.String(), wantExternalBase) {
		t.Fatalf("external bootstrap = %d %q", externalBootstrap.Code, externalBootstrap.Body.String())
	}
